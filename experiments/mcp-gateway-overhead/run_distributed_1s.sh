#!/usr/bin/env bash
#
# 1-Second Delay Mock Server Experiment.
#
# Proves gateway overhead is CONSTANT (~5ms), not proportional.
# If overhead stays ~5ms on top of ~1000ms tool calls → 0.5% not 50%.
#
# Same structure as run_distributed.sh but targets perf-mock-server-1s
# (RESPONSE_DELAY=1s per tool call).
#
# Usage:
#   ./run_distributed_1s.sh              # all concurrency levels
#   ./run_distributed_1s.sh -v           # verbose (stream master logs)
...
RESULTS_DIR="${RESULTS_BASE}/${TIMESTAMP}_1s"
...
# fewer concurrency levels — 1s delay means much lower RPS per user.
# key levels: 2 (baseline), 32 (moderate), 128 (high), 256, 512 (peak).

CONCURRENCY_LEVELS=(2 4 8 16 32 64 128 256 512)
USERS_PER_WORKER=32
TEST_DURATION=300                    # 5 min measurement (same as 0ms experiment)
WARMUP=60                            # same warmup as 0ms experiment
CPS=0
...
generate_distributed_yaml() {
    ...
    if [[ "$target" == "server" ]]; then
        host_url="http://perf-mock-server-1s.performance-test.svc.cluster.local:8080"
        tool_prefix=""
        host_header=""
    else
        host_url="http://mcp-gateway-istio.gateway-system.svc.cluster.local:8080"
        tool_prefix="mock1s_"
        host_header="perf-mock-1s.mcp.local"
    fi
...
log "=== 1-Second Delay Mock Server Experiment ==="
log "purpose:       prove gateway overhead is constant (~5ms), not proportional"
log "server:        perf-mock-server-1s (10 tools, RESPONSE_DELAY=1s)"
...
# apply 1s mock server deployment (separate pod, separate image)
log "applying perf-mock-server-1s deployment..."
kubectl apply -f "${MOCK_SERVER_1S_DIR}/deployment.yaml"
log "waiting for perf-mock-server-1s rollout..."
kubectl rollout status deployment/perf-mock-server-1s -n "${NAMESPACE}" --timeout=60s 2>/dev/null || true
log ""

# also ensure the 0ms server is running (gateway needs it for existing registrations)
log "ensuring perf-mock-server (0ms) is running..."
kubectl apply -f "${MOCK_SERVER_0MS_DIR}/deployment.yaml"
log ""

# apply infrastructure for 1s server
log "applying infrastructure for 1s server (HTTPRoute + MCPServerRegistration + DestinationRule)..."
kubectl apply -f "${CONFIG_DIR}/infrastructure-1s.yaml"

# also ensure the 0ms infrastructure is there
kubectl apply -f "${CONFIG_DIR}/infrastructure.yaml"

log "waiting for MCPServerRegistration perf-mock-server-1s..."
for i in $(seq 1 30); do
    ready=$(kubectl get mcpserverregistration perf-mock-server-1s -n "${NAMESPACE}" -o jsonpath='{.status.conditions[?(@.type=="Ready")].status}' 2>/dev/null || echo "")
    tools=$(kubectl get mcpserverregistration perf-mock-server-1s -n "${NAMESPACE}" -o jsonpath='{.status.discoveredTools}' 2>/dev/null || echo "0")
...
# restart both servers for clean state
log "restarting perf-mock-server-1s..."
kubectl rollout restart deployment/perf-mock-server-1s -n "${NAMESPACE}" 2>/dev/null || true
kubectl rollout status deployment/perf-mock-server-1s -n "${NAMESPACE}" --timeout=60s 2>/dev/null || true
log "perf-mock-server-1s ready"
log "restarting perf-mock-server (0ms)..."
...
log "perf-mock-server (0ms) ready"
log ""

# save metadata
mkdir -p "${RESULTS_DIR}"
cat > "${RESULTS_DIR}/metadata.txt" <<EOF
experiment:      1s-delay mock server (gateway overhead = constant proof)
timestamp:       ${TIMESTAMP}
server:          perf-mock-server-1s (10 tools, RESPONSE_DELAY=1s)
concurrency:     ${CONCURRENCY_LEVELS[*]}
users_per_worker: ${USERS_PER_WORKER}
warmup:          ${WARMUP}s
measurement:     ${TEST_DURATION}s
cps:             ${CPS}
gateway_prefix:  mock1s_
gateway_host:    perf-mock-1s.mcp.local
tools:           alpha bravo charlie delta echo foxtrot golf hotel india juliet
hypothesis:      gateway overhead stays ~5ms regardless of backend latency
EOF

# archive configs
log "archiving configs and scripts..."
mkdir -p "${RESULTS_DIR}/configs"
cp "${CONFIG_DIR}/infrastructure-1s.yaml" "${RESULTS_DIR}/configs/"
cp "${CONFIG_DIR}/infrastructure.yaml" "${RESULTS_DIR}/configs/"
cp "${CONFIG_DIR}/template_distributed.yaml" "${RESULTS_DIR}/configs/"
cp "${MOCK_SERVER_1S_DIR}/deployment.yaml" "${RESULTS_DIR}/configs/deployment-1s.yaml"
cp "${MOCK_SERVER_1S_DIR}/main.go" "${RESULTS_DIR}/configs/main-1s.go"
cp "${SCRIPT_DIR}/locustfile.py" "${RESULTS_DIR}/configs/"
cp "${LOAD_TESTER_DIR}/mcp_client.py" "${RESULTS_DIR}/configs/"
cp "${SCRIPT_DIR}/run_distributed_1s.sh" "${RESULTS_DIR}/configs/"
...
for users in "${CONCURRENCY_LEVELS[@]}"; do
    ...
    log "  restarting perf-mock-server-1s (clean GC state)..."
    kubectl rollout restart deployment/perf-mock-server-1s -n "${NAMESPACE}" 2>/dev/null || true
    kubectl rollout status deployment/perf-mock-server-1s -n "${NAMESPACE}" --timeout=60s 2>/dev/null || true

    for target in "${TARGETS[@]}"; do
        job_name="dist1s-u${users}-${target}"
